...
from numbapro import vectorize, jit, autojit, guvectorize, njit, typeof, \
    cuda, float64
...
def gpu_core_loop(gVec_c, gHat_c, gVec_l, rMat_sc, bHat_l, ztol, brMat,
    ...
    gpu_core_loop_kernel.forall(result.shape[0])(dev_gVec_c, dev_rMat_sc, dev_bHat_l,
                                                 ztol, dev_nVec_l, num, dev_P0_l,
                                                 dev_rMat_d, dev_tVec_d, dev_result)

    dev_result.copy_to_host(ary=result)

def test_loop_kernel(gVec_c, rMat_sc, bHat_l, ztol, nVec_l, num, P0_l,
                     rMat_d, tVec_d, result):
...
@cuda.jit("float64[:,:], float64[:], float64[:], float64, float64[:], "
          "float64, float64[:], float64[:,:], float64[:,:], float64[:,:]")
def gpu_core_loop_kernel(gVec_c, rMat_sc, bHat_l, ztol, nVec_l, num, P0_l,
                         rMat_d, tVec_d, result):
